...
public class Student {
	...
	public HashMap<String, Integer> getSemestersByYearAndSemester() {
		...
		int sequenSem = 1;
		for (Course bbb : aaa) {
			if (!semestersByYearAndSemester.containsKey(bbb.getYearTaken() + bbb.getSemesterCourseTaken())) {
				semestersByYearAndSemester.put(bbb.getYearTaken() + bbb.getSemesterCourseTaken(), sequenSem);
				sequenSem++;
			}
		}
		...
		for (int value : aaa.values()) {
			if (value == semester) {
				...
				for (Course courseTaken : coursesTaken) {
					String whenStudy = courseTaken.getYearTaken() + courseTaken.getSemesterCourseTaken();
