...
public class HGUCoursePatternAnalyzer {
	...
	public void run(String[] args) {
		...
		try {
			...
			/*
			 * if(args.length<1) throw new NotEnoughArgumentException(); } catch
			 * (NotEnoughArgumentException e) { System.out.println(e.getMessage());
			 * System.exit(0); }
			 */
	...
	private HashMap<String, Student> loadStudentCourseRecords(ArrayList<CSVRecord> lines) {
		...
		/*
		 * 
		 * for(int i=0; i < lines.size();i++) { String line = lines.get(i); String[]
		 * names = line.split(("\\s,\\s")); String name = names[0]; String course =
		 * names[5]; Student stustu = new Student(name); if(!stu.contains(stustu)) {
		 * stu.add(stustu); if(loadStuCourse.containsKey(stu.contains(stustu))) {
		 * 
		 * }else {
		 * 
		 * 
	 ...
	 * 0001,14,1,9 0001,14,2,8 .... 0001,14,1,9 => this means, 0001 student
	 * registered 14 semeters in total. In the first semeter (1), the student took 9
	 * courses.
	 * 
